...
当然，对这个场景需要考虑其后是否有后继结点，如果没有后继结点，会容易产生问题。
...
```
封装函数：`InsertNextDNode(DNode *p,DNode *s)`，对双链表的插入分为：按位序插入和指定节点前插操作、指定节点后插，都是可以直接调用该基础操作。

1. 按位序插入，以头结点开始，找到一个位置i的前驱结点，对前驱结点进行后插操作；

2. 指定节点前插操作，对这个结点的前一位，进行后插操作。
...
```c
bool IssTail(LinkList L, LNode *p)
{
    if (p->next == L) {
        return true;
    } else {
...
如何对一个双链表判空？头节点的`next`指针指向自己，那即为空
```c
// 双链表判空
...
- **链表**：插入/删除元素只需修改指针即可，时间复杂度 `O(n)`，时间开销主要来自查找目标元素，查找元素的时间代价更高
