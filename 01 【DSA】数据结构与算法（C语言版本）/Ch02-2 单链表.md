...
![](img/Ch02%20线性表/10%20单链表的节点.jpg)
...
![](img/Ch02%20线性表/11%20链表的初始化%20是否带头结点.jpg)
...
![](img/Ch02%20线性表/12%20链表插入%20给定位序%201.jpg)

3. 新结点后继为所寻结点的后继，新结点成为所寻结点的后继。

![](img/Ch02%20线性表/13%20链表插入%20给定位序%202.jpg)
...
![](img/Ch02%20线性表/14%20链表插入%20某个指定结点后插元素.jpg)
...
常规做法，是遍历找到那个结点的前驱结点，对前驱节点执行插入操作。

![](img/Ch02%20线性表/15%20链表插入%20某个指定结点前插元素%20常规.jpg)
...
![](img/Ch02%20线性表/16%20链表插入%20某个指定结点前插元素%20Swap.jpg)
...
看上去是一种在指定节点的后向插入，但是交换完数据后，后继变成了前驱。  
该算法的时间复杂度是`O(1)`.
...
```
方法：  
以带头节点为例，头结点可以看作“第0个”结点，找到第 i-1 个结点，将其指针指向第 i+1 个结点，并释放第 i 个结点。
...
![](img/Ch02%20线性表/18%20链表删除（给定位序）.png)

## 2.指定结点的删除
有两种方法，与"在某个指定结点前插元素"类似，

### 方法1：常规遍历
传入头指针 `L` ，循环寻找 `p` 的前驱结点


### 方法2：交换结点
偷天换日（类似于结点前插的实现）

![](img/Ch02%20线性表/19%20链表删除（给定结点）.png)
...
以上的时间复杂度是`O(1)`。  
特殊的，如果p是最后一个结点，那么只能从表头开始依次寻找p的前驱，时间复杂度 `O(n)`。
...
尾插法，十分适用于[链表逆置](Ch02-4%20链表逆置.md)。
