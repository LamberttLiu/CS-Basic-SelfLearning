...
![](img/Ch03%20栈与队列/18%20顺序队列初始化和判空.jpg)
...
## 4. 入队操作
- **带头结点**的链表形式
![](img/Ch03%20栈与队列/25%20入队链式队列.jpg)
```c
