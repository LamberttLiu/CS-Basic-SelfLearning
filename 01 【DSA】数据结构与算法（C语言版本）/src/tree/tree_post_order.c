...
void PostOrderTraverse(BiTree binaryTree, int* array, int* pos)
{
    if (binaryTree == NULL) {
        return;
    }

    PostOrderTraverse(binaryTree->lchild, array, pos);
    PostOrderTraverse(binaryTree->rchild, array, pos);
...
{
    int pos = 0;
    PostOrderTraverse(binaryTree, array, &pos);
