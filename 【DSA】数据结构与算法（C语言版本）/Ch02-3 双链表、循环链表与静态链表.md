...
![](img/02_线性表/29%20循环单链表%20空表.jpg)
...
如何对一个循环双链表判空？与循环单链表一样，头节点的`next`指针指向自己，那即为空。
